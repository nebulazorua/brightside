...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'saving-face' | 'parasitic-routine' | 'personal-space-invasion' | 'personal-fuck-you' | 'grey-matters':
				trace(SONG.song.toLowerCase() + '/dialogue');
				dialogue = CoolUtil.coolTextFile(Paths.txt(SONG.song.toLowerCase() + '/dialogue'));
				trace(Assets.getText(Paths.txt(SONG.song.toLowerCase() + '/dialogue')).trim());
			case 'reign-of-apathy':
				dialogue = CoolUtil.coolTextFile(Paths.txt('reign-of-apathy/dialogue' + Std.string(storyDifficulty) ));
		}

		if (SONG.song.toLowerCase() == 'spookeez' || SONG.song.toLowerCase() == 'monster' || SONG.song.toLowerCase() == 'south' || SONG.song.toLowerCase()=='you-are-a-fool' )
		{
			curStage = "spooky";
			halloweenLevel = true;
		...
		else if (SONG.song.toLowerCase() == 'thorns')
		{
		...
		else if (SONG.song.toLowerCase() == 'reign-of-apathy' || SONG.song.toLowerCase() == 'grey-matters'){
			defaultCamZoom = 0.9;
			curStage = 'spacebad';
			var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('corruptback'));
			bg.antialiasing = true;
			bg.scrollFactor.set(0.9, 0.9);
			bg.active = false;
			add(bg);

			var stageFront:FlxSprite = new FlxSprite(-650, 600).loadGraphic(Paths.image('corruptfront'));
			...
			defaultCamZoom = 0.9;
			curStage = 'brain';
			var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('brainback'));
			bg.antialiasing = true;
			bg.scrollFactor.set(0.9, 0.9);
			bg.active = false;
			add(bg);

			var stageFront:FlxSprite = new FlxSprite(-650, 600).loadGraphic(Paths.image('brainfront'));
			stageFront.setGraphicSize(Std.int(stageFront.width * 1.1));
			stageFront.updateHitbox();
			stageFront.antialiasing = true;
			stageFront.scrollFactor.set(0.9, 0.9);
		...
		else if (SONG.song.toLowerCase() == 'saving-face' || SONG.song.toLowerCase() == 'parasitic-routine'){
			defaultCamZoom = 0.9;
			curStage = 'space';
			var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('spaceback'));
			bg.antialiasing = true;
			bg.scrollFactor.set(0.9, 0.9);
			bg.active = false;
			add(bg);

			var stageFront:FlxSprite = new FlxSprite(-650, 600).loadGraphic(Paths.image('spacefront'));
			stageFront.setGraphicSize(Std.int(stageFront.width * 1.1));
			stageFront.updateHitbox();
			stageFront.antialiasing = true;
			stageFront.scrollFactor.set(0.9, 0.9);
		...
		healthBar = new FlxBar(healthBarBG.x + 4, healthBarBG.y + 4, FlxG.save.data.opponent ? LEFT_TO_RIGHT : RIGHT_TO_LEFT, Std.int(healthBarBG.width - 8), Std.int(healthBarBG.height - 8), this,
			'health', 0, 2);
		...
		kadeEngineWatermark = new FlxText(4,FlxG.height - 4,0,SONG.song + " " + (storyDifficulty == 2 ? "Hard" : storyDifficulty == 1 ? "Normal" : "Easy") + " - KE " + MainMenuState.kadeEngineVer, 16);
		...
		if (isStoryMode)
		{
			switch (curSong.toLowerCase())
			{
				...
				case 'thorns':
					schoolIntro(doof);
				case 'saving-face' | 'parasitic-routine' | 'grey-matters' | 'reign-of-apathy' | 'personal-space-invasion' | 'personal-fuck-you':
					bsIntro(doof);
				default:
					startCountdown();
			}
	...
	private function generateSong(dataPath:String):Void
	{
		...
		for (section in noteData)
		{
			...
			for (songNotes in section.sectionNotes)
			{
				...
				var swagNote:Note = new Note(daStrumTime, daNoteData, oldNote, false, songNotes[3]==null?"None":songNotes[3]);
				...
				for (susNote in 0...Math.floor(susLength))
				{
					...
					sustainNote.display = swagNote.display;
					sustainNote.scrollFactor.set();
					unspawnNotes.push(sustainNote);

					sustainNote.mustPress = gottaHitNote;

					if (FlxG.save.data.opponent && !sustainNote.mustPress || !FlxG.save.data.opponent && sustainNote.mustPress)
					{
				...
				if (FlxG.save.data.opponent && !swagNote.mustPress || !FlxG.save.data.opponent && swagNote.mustPress)
				{
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			if (FlxG.save.data.opponent && player==0 || !FlxG.save.data.opponent && player == 1)
			{
	...
	override public function update(elapsed:Float)
	{
		...
		if(SONG.song.toLowerCase()=='you-are-a-fool' || FlxG.save.data.moveNotes ){
			for(str in playerStrums){
				str.angle = 15*Math.cos((timer*2)+str.ID*2);
				str.y = strumLine.y+(40*Math.sin((timer*2)+str.ID*2));
				str.x = str.health+(100*Math.cos(((timer*4)+str.ID/2)));
			}
		}
		if (FlxG.save.data.accuracyDisplay)
		{
				...
				notes.forEachAlive(function(daNote:Note)
				{
					...
					if(SONG.song.toLowerCase()=='you-are-a-fool' || FlxG.save.data.moveNotes){
						if(daNote.mustPress){
							...
							daNote.x = daNote.health+(100*Math.cos(((timer*4)+daNote.noteData/2)));
						}
					}
					//trace(daNote.y);
	...
	function endSong():Void
	{
		...
		if (SONG.song.toLowerCase()=='you-are-a-fool' && !FlxG.save.data.yaaf){
			trace("YOU ARE A FOOL LOL");
			FlxG.save.data.yaaf=true;
			FlxG.switchState(new RewardSubstate());
		} else if (isStoryMode)
		{
			...
			if (storyPlaylist.length <= 0 || storyDifficulty!=2 && SONG.song.toLowerCase()=='reign-of-apathy' )
			{
				if(SONG.song.toLowerCase()=='personal-space-invasion'){
					trace("KILLED DA PARASITE");
					FlxG.save.data.finishedBrightsideStory=true;
				}
