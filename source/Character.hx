...
class Character extends FlxSprite
{
	...
	public function new(x:Float, y:Float, ?character:String = "bf", ?isPlayer:Bool = false)
	{
		...
		switch (curCharacter)
		{
			...
			case 'dad':
				...
				playAnim('idle');
			case 'brightside-masked':
				// DAD ANIMATION LOADING CODE
				tex = Paths.getSparrowAtlas("BrightsideMask");
				frames = tex;
				...
				addOffset('idle',0,10);
				addOffset("singUP", 50, 45);
				addOffset("singRIGHT", 0, -10);
				addOffset("singLEFT", 0, -40);
				addOffset("singDOWN", 40, -190);
				playAnim('idle');
				setGraphicSize(Std.int(width * .75));
				updateHitbox();
			case 'parasite':
				// DAD ANIMATION LOADING CODE
				tex = Paths.getSparrowAtlas("Parasite");
				frames = tex;
				animation.addByPrefix('idle', 'idle', 24, false);
				animation.addByPrefix('singUP', 'up', 24, false);
				animation.addByPrefix('singRIGHT', 'right', 24, false);
				animation.addByPrefix('singDOWN', 'down', 24, false);
				animation.addByPrefix('singLEFT', 'left', 24, false);
				animation.addByPrefix('shittyEmoji','yucky',24,false);
			...
			case 'brightside':
				// DAD ANIMATION LOADING CODE
				tex = Paths.getSparrowAtlas("Brightside");
				frames = tex;
				animation.addByPrefix('idle', 'sadidle', 24, false);
				animation.addByPrefix('singUP', 'sadup', 24, false);
				animation.addByPrefix('singRIGHT', 'sadright', 24, false);
				animation.addByPrefix('singDOWN', 'saddown', 24, false);
				animation.addByPrefix('singLEFT', 'sadleft', 24, false);

				addOffset('idle',0,0);
				addOffset("singUP", -1,25);
			...
			case 'vase':
				// DAD ANIMATION LOADING CODE
				tex = Paths.getSparrowAtlas("VaseSheet");
				frames = tex;
				animation.addByPrefix('idle', 'idle', 24, false);
				animation.addByPrefix('singUP', 'up', 24, false);
				animation.addByPrefix('singRIGHT', 'right', 24, false);
				...
				playAnim('idle');
				setGraphicSize(Std.int(width*.45));
			case 'spooky':
		...
		if (FlxG.save.data.opponent && !isPlayer && !curCharacter.startsWith('gf') || !FlxG.save.data.opponent && isPlayer)
		{
			flipX = !flipX;
	...
	override function update(elapsed:Float)
	{
		if (!isPlayer)
		{
			...
			if (curCharacter == 'dad')
				dadVar = 6.1;
			if(curCharacter == 'parasite' || curCharacter == 'vase')
				dadVar = 10;
